...
class SeparateBN(nn.Layer):
    def __init__(self, bn, num_domains=2):
        ...
        self.bns = nn.LayerDict({str(i): deepcopy(bn) for i in range(num_domains)})
    ...
    def forward(self, input, domain_indices):
        if isinstance(domain_indices, paddle.Tensor):
            ...
            domain_indices = paddle.ones(batch_size, dtype=paddle.int) * domain_indices
            domains = domain_indices.unique()
        else:
        ...
        return paddle.cat(batches, dim=0)
