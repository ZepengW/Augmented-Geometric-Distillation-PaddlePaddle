...
class Base(nn.Layer):
    ...
    def __init__(self, block, layers, zero_init_residual=False, groups=1, width_per_group=64,
        ...
        if zero_init_residual:
            for m in self.modules():
                # modified from paddlevision version
                nn.init.constant_(m.bn3.weight, 0)

        if pretrained:
            # assert osp.isfile(
            #     osp.expanduser('~/.cache/paddle/checkpoints/resnet50-19c8e357.pth')), "paddle pretrained model " \
            #                                                                          "doesn't exists."
            ...
            weight_path = get_weights_path_from_url(model_urls['resnet50'][0],
