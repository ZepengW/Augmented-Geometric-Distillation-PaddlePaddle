...
class TriHardPlusLossEnhanced(object):
    ...
    def hard_example_mining(dist_mat, labelsA, labelsB):
        ...
        # `dist_ap` means distance(anchor, positive)
        dist_ap, relative_p_inds = paddle.max(dist_mat * is_pos.to(paddle.float32), 1, keepdim=True)
        # `dist_an` means distance(anchor, negative)
        dist_an, relative_n_inds = paddle.min(dist_mat + is_pos.to(paddle.float32) * 1e3, 1, keepdim=True)
        # shape [N]
        dist_ap = dist_ap.squeeze(1)
        dist_an = dist_an.squeeze(1)
...
class MMDLoss2(object):
    ...
    def __call__(self, x, y, sigmas=(1, 5, 10), normalize=False):
        ...
        if len(sigmas) == 0:
            mean_dist = paddle.mean(paddle.pow(dist(x, y), 2))
    ...
    def _get_mmd_factor(self, sigmas, device):
        sigmas = paddle.Tensor(sigmas)[:, None, None].float()
        sigmas = -1 / (2 * sigmas)
        return sigmas
