...
def build_optimizer(backbone, classifier, args):
    # if isinstance(backbone, paddle.nn.DataParallel):
    ...
    if args.optimizer == 'Adam':
        optimizer_main = paddle.optimizer.Adam(parameters=param_groups, learning_rate=args.learning_rate)
        print("Build Adam optimizer for backbone.")
    elif args.optimizer == 'SGD':
        optimizer_main = paddle.optimizer.SGD(parameters=param_groups, learning_rate=args.learning_rate, weight_decay=5e-4)
